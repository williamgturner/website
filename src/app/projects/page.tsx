...
export default function Projects() {
  const postsDir = path.join(process.cwd(), "projects");
  const filenames = fs
  ...
  const posts: PostMeta[] = filenames.map((filename) => {
    ...
    if (frontmatter.date) {
      if (/^\d{4}$/.test(frontmatter.date)) {
        // Year-only format
        date = new Date(`${frontmatter.date}-01-01`);
        isYearOnly = true;
      } else {
        ...
        {posts.map((post) => (
          <li key={post.slug}>
            <div className="flex flex-col">
              <Link href={`/projects/${post.slug}`} className="hover:bg-[orange]">
                {post.title}
              </Link>
              {post.date && (
